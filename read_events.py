...
event_city_fig = rd.info_draw(event_city_series, 'event_venue_city')
...
event_group_series = rd.info_split_merge(event_info, 'group.name')
event_group_fig = rd.info_draw(event_group_series, 'event per group')
event_groupwho_series = rd.info_split_merge(event_info, 'group.who')
event_groupwho_fig = rd.info_draw(event_groupwho_series, 'events number per who join')
event_created_fig = rd.info_groupedby_created(event_info, gap='month')
plt.show()

# rd.multiSplit(event_info,['fee.required','venue.state','venue.city','visibility'])