...
Art_arange = [0, 50, 150, inf]
[Arts_fig, Arts_ranges, Arts_eg] = event_per_group(Arts_event, Arts_group, Art_arange,
                                                   'event per group (Arts category)')
...
[Career_fig, Career_ranges, Career_eg] = event_per_group(Career_event, Career_group, Career_arange,
                                                         'event per group (Career category)')
...
Book_arange = [0, 20, 100, inf]
[Book_fig, Book_ranges, Book_eg] = event_per_group(Book_event, Book_group, Book_arange,
                                                   'event per group (Book category)')
Arts_eg_pie = event_menber_pie(Arts_eg, 50, 'event and member percentage (Arts category)')
Career_eg_pie = event_menber_pie(Career_eg, 40, 'event and member percentage (Career category)')
Book_eg_pie = event_menber_pie(Book_eg, 13, 'event and member percentage (Book category)')
print(Arts_ranges)
print(Career_ranges)
print(Book_ranges)
Arts_ranges.to_csv(Const.ART_EVENT_PATH, sep=',')
Career_ranges.to_csv(Const.CAREER_EVENT_PATH, sep=',')
Book_ranges.to_csv(Const.BOOK_EVENT_PATH, sep=',')

Arts_event.to_csv(Const.ART_EVENT_DATA_PATH, sep=',', index=False)
Career_event.to_csv(Const.CAREER_EVENT_DATA_PATH, sep=',', index=False)
